...
export default function HerbalProductsSection({ locale }: { locale: string }) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      // Find category IDs that are of type 'herbal'
      const herbalCatIds = allCategories
        .filter((cat: Category) => cat.type === 'herbal')
      ...
      <div className="container mx-auto px-4 md:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16">
          <span className="text-xs uppercase font-extrabold tracking-widest text-gold-dark px-3 py-1 bg-gold-lighter/40 rounded-full border border-gold-light/20">
            {locale === 'ur' ? 'خالص اور نامیاتی' : 'Organic Wellness'}
          </span>
